...
import { Table, Input, Select, Button, Tag, Space, Badge, Tooltip, Dropdown, DatePicker, InputNumber } from 'antd';
import { 
  EyeOutlined, 
  MessageOutlined, 
  DownloadOutlined, 
...
export function ShippersList({ theme = 'dark', onViewShipper }: ShippersListProps) {
  ...
  const columns = [
    {
      ...
      render: (id: string) => (
        <div>
          ...
          <Tag color="blue" style={{ fontSize: '11px', marginTop: '4px' }}>Shipper</Tag>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_: any, record: Shipper) => (
        <span style={{ fontSize: '13px', color: textSecondary }}>
          <span style={{ color: textPrimary, fontWeight: 600 }}>B:{record.metrics.bookings}</span>
          {' • '}
          <span style={{ color: '#10B981' }}>C:{record.metrics.completed_shipments}</span>
        </span>
      ),
    },
    {
    ...
    <div>
      ...
      <div style={{ 
        ...
        <Select
          ...
          onChange={(value) => setParams(prev => ({ ...prev, has_acs: value, offset: 0 }))}
          options={[
        ...
        <InputNumber
          ...
          onChange={(value) => setParams(prev => ({ ...prev, min_balance: value || undefined, offset: 0 }))}
        />
        <Button 
          icon={<FilterOutlined />}
