...
import { Table, Input, Select, Button, Tag, Space, Badge, Tooltip, Dropdown } from 'antd';
import { 
  EyeOutlined, 
  MessageOutlined, 
  DownloadOutlined, 
...
export function DriversList({ theme = 'dark', onViewDriver }: DriversListProps) {
  ...
  const mockDrivers: Driver[] = [
    ...
    },
    {
      ...
      last_location: { city: 'Vijayawada', lat: 16.5062, lng: 80.6480, timestamp: '2025-12-04T08:45:00Z' },
      trips_30d: 28,
      behaviour_score: 95,
      acs_flags_count: 0,
      trust_score: 95,
    },
    {
      ...
      last_location: { city: 'Guntur', lat: 16.3067, lng: 80.4365, timestamp: '2025-12-03T18:30:00Z' },
      trips_30d: 15,
      behaviour_score: 68,
      acs_flags_count: 2,
      trust_score: 68,
  ...
  const columns = [
    {
      ...
      render: (id: string) => (
        <div>
          ...
          <Tag color="orange" style={{ fontSize: '11px', marginTop: '4px' }}>Driver</Tag>
        </div>
      ),
    },
    {
            ...
            <Tooltip title={record.operators.map(o => o.name).join(', ')}>
              <div>
                ...
                {record.operators.length > 1 && (
                  <Tag style={{ fontSize: '11px' }}>+{record.operators.length - 1}</Tag>
                )}
              </div>
            </Tooltip>
          ) : (
      ...
      render: (expiry: string) => (
        <div>
          {isDLExpired(expiry) ? (
            ...
            <Tooltip title={`Expires on ${dayjs(expiry).format('DD MMM YYYY')}`}>
              <Tag color="orange">Expiring Soon</Tag>
            </Tooltip>
          ) : (
            <span style={{ color: textSecondary }}>{dayjs(expiry).format('DD MMM YYYY')}</span>
          )}
        </div>
      ),
    },
      ...
      render: (score: number, record: Driver) => (
        <div>
          ...
          {record.acs_flags_count > 0 && (
            <Badge count={record.acs_flags_count} style={{ backgroundColor: '#EF4444' }} />
          )}
        </div>
      ),
    },
          ...
          <Dropdown
            menu={{
              items: [
                ...
                { key: 'assign', label: 'Assign to Operator' },
                { key: 'unlink', label: 'Unlink from Operator' },
                { key: 'block', label: 'Block Driver', danger: true },
              ],
            }}
          >
    ...
    <div>
      ...
      <div style={{ 
        ...
        <Select
          ...
          onChange={(value) => setParams(prev => ({ ...prev, operator_id: value, offset: 0 }))}
          options={[
        ...
        <Select
          ...
          onChange={(value) => setParams(prev => ({ ...prev, availability: value, offset: 0 }))}
          options={[
        ...
        <Select
          ...
          onChange={(value) => setParams(prev => ({ ...prev, has_flags: value, offset: 0 }))}
          options={[
        ...
        <Button 
          icon={<FilterOutlined />}
          onClick={() => setParams({ limit: 25, offset: 0, sort: 'last_ping', order: 'desc' })}
        >
          Clear Filters
        </Button>
      </div>
