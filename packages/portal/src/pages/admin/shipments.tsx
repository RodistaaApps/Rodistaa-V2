...
import { 
  Table, Card, Input, Select, Button, Tag, Space, Badge, Tooltip, Progress, message 
} from 'antd';
import {
  SearchOutlined,
  FilterOutlined,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (truckId: string) => (
        <span style={{ fontFamily: 'monospace', color: textPrimary }}>{truckId}</span>
      ),
    },
    {
      title: 'Driver',
    ...
    },
    {
      ...
      render: (uploaded: boolean) => (
        uploaded ? (
          ...
          <CloseCircleOutlined style={{ color: textSecondary, fontSize: '18px' }} />
        )
      ),
    },
    {
      ...
      render: (state: string, record: Shipment) => (
        <div>
          ...
          <div style={{ fontSize: '11px', color: textSecondary, marginTop: '4px' }}>
            ₹{record.advance_paid.toLocaleString()} / ₹{record.freight_amount.toLocaleString()}
          </div>
        </div>
      ),
    },
