...
function mapRowToTruck(row: any): Truck {
  return {
    ...
    bsType: row.bs_type || 'BS6',
    chassisNumber: row.chassis_number || '',
    vehicleType: 'HGV',
    tonnage: row.tonnage || row.capacity_tons || 0,
    status: row.status as TruckStatus,
    lastInspectionAt: row.last_inspection_at ? new Date(row.last_inspection_at) : undefined,
    nextInspectionDue: row.next_inspection_due ? new Date(row.next_inspection_due) : undefined,
    documents: {
      rcExpiry: row.rc_expiry ? new Date(row.rc_expiry) : undefined,
      fitnessExpiry: row.fitness_expiry ? new Date(row.fitness_expiry) : undefined,
      permitExpiry: row.permit_expiry ? new Date(row.permit_expiry) : undefined,
      insuranceExpiry: row.insurance_expiry ? new Date(row.insurance_expiry) : undefined,
      pollutionExpiry: row.pollution_expiry ? new Date(row.pollution_expiry) : undefined,
    },
